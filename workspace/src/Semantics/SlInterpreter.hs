...
eval (e1 :-: e2) = opNum (-) (-) e1 e2
eval (e1 :*: e2) = opNum (*) (*) e1 e2
eval (e1 :/: e2) = do -- Divisão precisa checar zero!
    v1 <- eval e1
    v2 <- eval e2
    case (v1, v2) of
...
eval (EStruct nome exprs) = do
    ...
    -- Precisaríamos checar os nomes dos campos aqui, mas vamos simplificar
    -- assumindo que o TypeChecker já garantiu a ordem.
    -- Vamos precisar dos nomes dos campos da definição do struct para criar o Map.
    env <- get
...
-- Nota: Isso precisa de mais lógica para escopo local, faremos no próximo passo.
...
-- Reduzem a repetição de código para Int/Float
...
-- Nota: Estamos tratando apenas o caso base onde o array é uma variável (EVar).
-- Arrays multidimensionais (v[i][j]) exigiriam recursão mais complexa.
...
exec (SAssign (EField exprObj campo) exprVal) = do
    -- Atribuição em struct (p.x = 10) é mais chato:
    -- Precisamos ler o struct, mudar o campo, e salvar o struct de volta.
        ...
        Just (Dfunc _ _ params _ bloco) -> do
            ...
            if length valsArgs /= length params
                then err "Numero incorreto de argumentos"
                    ...
                    -- ATENÇÃO: Aqui estamos usando escopo dinâmico misturado com estático simplificado.
                    -- O correto seria criar um escopo limpo apenas com globais + params.
                    -- Para simplificar, vamos apenas sobrescrever vars, mas mantendo o que já existia (permite recursão).
