...
import Control.Monad (zipWithM_)  -- <--- O IMPORT QUE FALTAVA
...
unify t1 t2 = 
    ...
    then return ()
        ...
        -- Se um deles for uma variável de tipo genérica (TVar "a"), 
        -- por enquanto vamos aceitar a unificação (match simples).
        -- Num compilador real (Hindley-Milner completo), aqui faríamos substituição.
        -- Para este trabalho, aceitar a igualdade estrutural ou TVar costuma bastar 
        -- para passar nos testes de "compatibilidade".
        (TVar _, _) -> return () 
        (_, TVar _) -> return ()
...
typeCheckExp (EStruct nomeStruct exprs) = do
    ...
    case lookupStruct nomeStruct env of
        ...
        Just fieldsList -> do
            -- Agora fieldsList é [(String, Type)].
...
typeCheckExp (EField expr nomeCampo) = do
    ...
    case tipoObj of
        ...
        -- Caso 2: É um Array e estamos pedindo ".size" (O FIX!)
        TVector _ -> 
            if nomeCampo == "size"
            then return TInt
...
-- === ADICIONE ISTO EM SlTypeChecker.hs ===
...
checkStmt (SIf cond blocoThen blocoElse) = do
    ...
    checkBlock blocoThen
    checkBlock blocoElse
    return TVoid
...
checkBlock (stmt:resto) = 
    case stmt of
        -- Caso 1: Let com tipo explícito (Já existia)
        SLet var tipo (Just expr) -> do
            tExpr <- typeCheckExp expr
            unify tipo tExpr
            local (extendVar var tipo) (checkBlock resto)
            
        -- Caso 2: Let sem inicialização (Já existia)
        SLet var tipo Nothing -> do
            local (extendVar var tipo) (checkBlock resto)

        -- === NOVO: Caso 3: Inferência (let x = expr;) ===
        SLetInfer var expr -> do
            -- 1. Descobrimos o tipo da expressão da direita
            tExpr <- typeCheckExp expr
            
            -- 2. Estendemos o ambiente assumindo que 'var' tem o tipo 'tExpr'
            --    e continuamos verificando o resto do bloco.
            local (extendVar var tExpr) (checkBlock resto)

        -- Caso 4: Return (Novo caso para não ser confundido com o LetInfer)
        SReturn expr -> do
            t <- typeCheckExp expr
            return t

        -- Outros comandos (Já existia)
        _ -> do
            _ <- checkStmt stmt
            checkBlock resto
